...
- 이름을 use 로 시작하고, 그 뒤 첫글자는 대문자여야 한다. > 리액트가 hook 으로 인지하기 위해 필요 
- 컴포넌트나 다른 커스텀 훅의 최상위에서 호출한다. (조건문, 반복문, 이벤트 핸들러 안에서 호출 X) (early return 이 실행되기 전) > 다른 hook 들과 순서와 상태를 올바르게 유지하기 위해 필요
