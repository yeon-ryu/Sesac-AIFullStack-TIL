...
// css 적용할 클래스명 추가/삭제/토글
요소.classList.add('삽입하고 싶은 클래스명');
요소.classList.remove('삭제하고 싶은 클래스명');
요소.classList.toggle('있으면 삭제하고 없으면 추가하고 싶은 클래스명');
...
XSS 공격 : 악의적인 공격자가 웹사이트에 악성 스크립트를 삽입하여 다른 사용자의 브라우저에서 실행하게 만드는 것  
...
- 사용자 정의 어트리뷰트 : data-이름 형태로 어트리뷰트 작성
- 프로퍼티 접근 : 요소.dataset.(카멜 케이스로 변경한 data- 를 제외한 속성의 키 값);
```html
<ul class="board-list">
...
<script>
    ...
    $boardList.onclick = (e) => {
        ...
        // 요소.dataset.(카멜 케이스로 변경한 속성의 키 값);
        const boardId = e.target.dataset.boardId; // data-board-id
        const categoryName = e.target.dataset.categoryName; // data-category-name
        
        // 숫자로 써놨어도 가져올 때 문자로 가져온다.
        console.log(`선택한 게시글 ${boardId}, ${categoryName}`);

        // 사용자 정의 속성의 값을 추가하거나 변경
        e.target.dataset.readStatus = 'read'; // data-read-status="read" 속성 추가
        // 삭제
        delete e.target.dataset.categoryName;
    };
</script>
```
e.target.tagName : 대문자 태그명으로 값이 나온다.

&nbsp;

### 사용자 정의 dataset 실무 예제
상품 상세나 서버에 보낼 필요한 값으로 사용자 정의 속성을 사용한다.
```html
<!--
  [HTML]
  각 상품(li)마다 눈에 보이지 않는 상품 ID와 가격 정보를
  'data-product-id'와 'data-price' 어트리뷰트에 숨겨두었습니다.
-->
<ul class="product-list">
    <li class="product-item" data-product-id="p-001" data-price="15000">
        <h3>상품 A</h3>
        <p>15,000원</p>
    </li>
    <li class="product-item" data-product-id="p-002" data-price="22000">
        <h3>상품 B</h3>
        <p>22,000원</p>
    </li>
    <li class="product-item" data-product-id="p-003" data-price="8500">
        <h3>상품 C</h3>
        <p>8,500원</p>
    </li>
</ul>
...
<script>
    ...
    $productList.addEventListener('click', e => {
        const clickedItem = e.target.closest('.product-item');
        if(!clickedItem) return;

        const productId = clickedItem.dataset.productId; // data-product-id
        const price = clickedItem.dataset.price; // data-price

        $infoPanel.innerHTML = `선택된 상품 ID: <span>${productId}</span><br>
...
- [DOM (Document Object Model)](#dom-document-object-model)
      - [자주 사용하게 되는 메서드](#자주-사용하게-되는-메서드)
  - [요소 선택](#요소-선택)
      - [일반 배열로 변환](#일반-배열로-변환)
  - [노드의 부모/자식/형제 요소 선택](#노드의-부모자식형제-요소-선택)
    - [자식 요소 반환](#자식-요소-반환)
    - [부모 요소 반환](#부모-요소-반환)
    - [형제 요소 반환](#형제-요소-반환)
  - [노드 조작](#노드-조작)
  ...
  - [DOM 조작과 추가](#dom-조작과-추가)
    - [innerHTML](#innerhtml)
    - [insertAdjacentHTML](#insertadjacenthtml)
    - [요소를 직접 생성 및 조합](#요소를-직접-생성-및-조합)
    - [노드 삽입/이동/교체/삭제](#노드-삽입이동교체삭제)
  - [Attribute 와 Property](#attribute-와-property)
    - [어트리뷰트 읽기/쓰기/확인/삭제](#어트리뷰트-읽기쓰기확인삭제)
    - [어트리뷰트와 프로퍼티의 차이](#어트리뷰트와-프로퍼티의-차이)
    - [사용자 정의 어트리뷰트](#사용자-정의-어트리뷰트)
    - [사용자 정의 dataset 실무 예제](#사용자-정의-dataset-실무-예제)
  - [CSS 조작](#css-조작)
      - [Tip](#tip)
    - [인라인 스타일로 적용](#인라인-스타일로-적용)
    - [클래스 제어](#클래스-제어)