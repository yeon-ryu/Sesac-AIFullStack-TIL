...
**XSS 보안 위험은 동일하게 존재**
키워드 참고 문서 : https://developer.mozilla.org/ko/docs/Web/API/Element/insertAdjacentHTML
```html
<!-- beforebegin -->
...
- **노드 이동 (appendChild)** : 이미 존재하는 노드를 원하는 위치에 추가하면 자동으로 이전 위치에서 삭제되며 이동된다. 같은 노드는 중복으로 존재할 수 없다. (복제하고 싶으면 cloneNode 를 사용해야한다.)
...
| Attribute | Property |
| --- | --- |
| HTML 태그에 부여된 속성 | HTML 을 브라우적 읽어 만든 DOM 객체의 속성 |
